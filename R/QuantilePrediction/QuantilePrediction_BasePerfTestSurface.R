...
for(r in 1:R){
  ...
  # layerSizes <- c(100, 100, 5)
  layerSizes <- c(50, 50, 5)
  # in_Ns <- c(Inf, 36, 8)
  # out_Ns <- c(36, 8, Inf)
  # in_Ns <- c(Inf, 16, 8)
  # out_Ns <- c(16, 8, Inf)
  ...
  layerDropouts <- c(rep(0.2, length(layerSizes)-1),0)
  layerTransforms <- c(lapply(seq_len(length(layerSizes)-1), function(x) nnf_leaky_relu), list(function(x) {nnf_softmax(x,2)}))
  baseModel <- constructFFNSB(inputSize, layerSizes, layerTransforms, in_Ns, out_Ns, layerDropouts)
  # baseModel <- constructFFNSF(inputSize, layerSizes, layerTransforms, in_Ns, out_Ns, layerDropouts)
...
metaModel <- MetaModel(baseModel, xtype_train, mesaParameterSize = 3, allowBias = T, pDropout = 0,  initMesaRange = 0, initMetaRange = 0.7)
